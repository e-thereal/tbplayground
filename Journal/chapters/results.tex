...
different combinations of modalities, with a comparison to four publicly
...
FLAIR MRIs, divided into 20 training cases for which ground truth segmentation
...
% TODO: if space is a concern just cite traboulsee2008
...
\cite{sudre2015,subbanna2015,guizard2015}) as reference points for comparison:
...
implemented in the lesion segmentation toolbox (LST) version 2.0.11, c) the
lesion prediction algorithm (LST-LPA) also implemented in the LST toolbox, and
d) Lesion-TOADS version 1.9 R \cite{shiee2010topology}. The Lesion-TOADS
software only takes T1w and FLAIR MRIs and has no tunable parameters, so we used
the default parameters to carry out the segmentations.
...
the modalities used. We carried out the segmentation using two combinations of
...
0.9$ for four modalities). LST-LGA and LST-LPA only take T1w and FLAIR MRIs as
...
the training and validation sets during training of a 7-layer CEN-s up to 500
epochs.
...
epochs, we decided to stop the training procedure at 500 epochs. The small size
of the challenge data sets did not allow us to use a dedicated validation set to
choose the number of epochs. Instead, we set the number of epochs to 2500, which
corresponds to roughly the same number of gradient updates compared to the
...
challenges, we used a CEN with only 3 layers on these data sets to reduce
...
method ranked 6th (2nd if only considering methods with only one submission) out
of 52 entries submitted to the challenge, outperforming the recent SLS by Roura
...
\end{tabular}
\end{center}
Note: Columns LTPR, LFPR, and VD show the average of the two raters in
percent. Last updated: Dec 15, 2015.
\end{table}
...
which have published summary of their mean DSC, LTPR and LFPR scores for both
...
an anisotropic filter size of \num{9x9x5}. We also ran the four competing
methods discussed in Section~III-B with Lesion-TOADS and the two LST methods
using the T1w and FLAIR images, and EMS using three (T1w, T2w, PDw) and all four
modalities in separate tests. A comparison of the segmentation accuracy of the
trained networks and competing methods is summarized in
Table~\ref{tab:results1}. All CEN architectures performed significantly better
than all other methods regardless of the input modalities, with LST-LGA being
the closest in overall segmentation accuracy. Comparing CEN to LST-LGA, the
improvements in the mean DSC scores ranged from 3 percentage points (pp) for the
3-layer CEN to 17\,pp for the 7-layer CEN with shortcut trained on all four
modalities. The improved segmentation performance was mostly due to an increase
in lesion sensitivity. LST-LGA achieved a mean lesion TPR of
\SI{37.50}{\percent}, compared to \SI{54.55}{\percent} produced by the CEN with
shortcut when trained on the same modalities, and \SI{62.49}{\percent} when
trained on all four modalities, while achieving a comparable number of lesion
false positives. The mean lesion FPRs and mean volume differences of LST-LGA and
the 7-layer CEN-s were very close, when trained on the same modalities, and the
CEN-s further reduced its FPR when trained on different modalities.
...
that can capture much larger lesions. Consequently, the
...
adding shortcut connections reduces the network's sensitivity to very small
...
detect large lesions (see Fig.~\ref{fig:large}).}
\label{fig:small}
\end{figure}
...
and FLAIR MRIs. The advantage extended to all lesion sizes when the CEN-s was
...
lesion TPR, especially for smaller lesions, while the number of false positives
